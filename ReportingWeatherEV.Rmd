...
restd <- df %>% summarise(sfat = sdv(FATALITIES),sinj = sdv(INJURIES),spdmg = sdv(PROPDMGMUS),scdmg =sdv(CROPDMGMUS))

table(resum,remean,restd)
...
par(mfrow=c(1,2))
with(sum_sm_df,plot(BGN_DATE,INJURIES, col="red",type="l",xlab="Year", ylab="Quanity",main="effects in the public health"))
abline(sum_sm_df$BGN_DATE,sum_sm_df$FATALITIES, col="orange")
with(sum_sm_df,plot(BGN_DATE,PROPDMGMUS,col="blue", xlab="Year", ylab="US Dollar Millions", main="Economic costs of Weathwr Events"))
abline(sum_sm_df$BGN_DATE,sum_sm_df$CROPDMGMUS,col="magenta")
