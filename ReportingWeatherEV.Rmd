...
resum <- df %>% summarise(fat = sum(FATALITIES),inj = sum(INJURIES),pdmg = sum(PROPDMG),cdmg =sum(CROPDMG))

remean <- df %>% summarise(mfat = mean(FATALITIES),minj = mean(INJURIES),mpdmg = mean(PROPDMG),mcdmg =mean(CROPDMG))

restd <- df %>% summarise(sfat = sdv(FATALITIES),sinj = sdv(INJURIES),spdmg = sdv(PROPDMG),scdmg =sdv(CROPDMG))

table(resum,remean,restd)

#sm_df <- group_by(df,year(df$BGN_TIME))
